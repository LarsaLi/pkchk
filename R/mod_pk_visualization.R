      ...
      shiny::moduleServer(id, function(input, output, session) {
        ...
        output$pk_spaghetti <- shiny::renderPlot({
          ...
          if (requireNamespace("ggplot2", quietly = TRUE)) {
            ...
            df$GROUP_COL <- if ("ARM" %in% names(df)) as.character(df$ARM) else if ("POPPKFL" %in% names(df)) as.character(df$POPPKFL) else "ALL"
            ...
            p <- ggplot2::ggplot(df, ggplot2::aes_string(x = "TIME_NUM", y = "AVAL_NUM", group = "USUBJID", color = "GROUP_COL")) +
              ggplot2::geom_line(alpha = 0.22) +
              ggplot2::geom_point(alpha = 0.5, size = 1.1) +
              ggplot2::theme_minimal() +
              ggplot2::labs(x = "TIME", y = "AVAL", color = "Group", title = "Concentration-Time Spaghetti")

            mean_df <- stats::aggregate(AVAL_NUM ~ TIME_NUM, data = df, FUN = function(x) mean(x, na.rm = TRUE))
            sd_df <- stats::aggregate(AVAL_NUM ~ TIME_NUM, data = df, FUN = function(x) stats::sd(x, na.rm = TRUE))
            ...
            p <- p +
              ggplot2::geom_ribbon(data = ov, ggplot2::aes_string(x = "TIME_NUM", ymin = "ymin", ymax = "ymax"), inherit.aes = FALSE, alpha = 0.15, fill = "#111827") +
              ggplot2::geom_line(data = ov, ggplot2::aes_string(x = "TIME_NUM", y = "mean"), inherit.aes = FALSE, linewidth = 0.9, color = "#111827")

            if (is.finite(lloq)) {
              p <- p + ggplot2::geom_hline(yintercept = lloq, linetype = 2, color = "#ef4444")
            }
            if (any(df$BLQ_MARK, na.rm = TRUE)) {
              p <- p + ggplot2::geom_point(data = df[df$BLQ_MARK, , drop = FALSE], shape = 4, stroke = 1.1, size = 2, color = "#dc2626")
            }
        ...
        output$dose_cmax_plot <- shiny::renderPlot({
          ...
          if ("EVID" %in% names(d)) d <- d[d$EVID == 0, , drop = FALSE]
          cmax <- stats::aggregate(AVAL ~ USUBJID, d, function(x) max(as.numeric(x), na.rm = TRUE))
          ...
          if (requireNamespace("ggplot2", quietly = TRUE)) {
            p <- ggplot2::ggplot(m, ggplot2::aes_string(x = "DOSE", y = "AVAL")) +
              ggplot2::geom_point(color = "#0ea5e9", alpha = 0.8) +
              ggplot2::theme_minimal() +
              ggplot2::labs(x = "DOSE", y = "Cmax", title = "DOSE vs Cmax")
            print(p)
          } else {
            graphics::plot(m$DOSE, m$AVAL, pch = 19, col = "#0ea5e9", xlab = "DOSE", ylab = "Cmax", main = "DOSE vs Cmax")
          }
        })
      })
    },
